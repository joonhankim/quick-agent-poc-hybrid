...
Azure AI Search 인덱스(`law-unified-index`)에 적재된 데이터는 **법제처 Open API**를 통해 수집한 대한민국 공공 법률 데이터입니다.

| 구분 | 건수 | 출처 |
| :--- | ---: | :--- |
| **법령** (법률·시행령·시행규칙) | 500건 | 법제처 Open API (법령 API) |
| **판례** | 5,000건 | 법제처 Open API (판례 API) |
| **합계** | **5,500건** | |

### 법령 데이터 (500건)

- 대한민국 현행 법률, 시행령, 시행규칙, 직제 등
- 건축법, 개인정보 보호법, 근로기준법, 상법, 민법 등 다양한 분야 포함
- 법령 본문 전문(全文)이 `content` 필드에 저장

### 판례 데이터 (5,000건)

- **최신성**: 2025년 12월 판결까지 포함
- **법원 분포**: 대법원(1,146건), 서울고등법원(109건), 서울중앙지방법원(37건) 등 전국 50개 이상 법원
- **사건 유형**: 세금, 부동산, 채권, 노동, 형사, 행정 등 전 분야
- 각 판례에 사건번호(`case_number`), 판결일(`decision_date`), 법원명(`court`) 메타데이터 포함

### 인덱스 스키마

| 필드 | 설명 | 예시 |
| :--- | :--- | :--- |
| `title` | 문서 제목 | 근로기준법, 손실보상금 |
| `content` | 본문 내용 | 법령 전문 또는 판결 요지 |
| `source` | 출처 구분 | 법제처_법령, 법제처_판례 |
| `category` | 문서 유형 | 법률, 판례 |
| `law_number` | 법률/조항 번호 | - |
| `case_number` | 사건번호 | 2024두66136 |
| `decision_date` | 판결일 | 2025.12.11 |
| `court` | 법원명 | 대법원 |
